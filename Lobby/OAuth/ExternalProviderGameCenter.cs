...
namespace Lobby.OAuth
{
    ...
    {
        public bool CheckAuth(string platformId, GameCenterAuth auth)
        {
                ...
                if (cert.Verify())
                {
                    ...
                    if (csp != null)
                    {
                        ...
                        if (csp.VerifyHash(hash, CryptoConfig.MapNameToOID("SHA256"), Convert.FromBase64String(auth.Signature)))
                        {
                            ...
                            //Logger.Info(string.Format("GameCenter TokenCheck success, usn:{0}, {1}", venderUID, auth.ToString()));
                            return true;
                        }
                        else
                        {
            ...
            Log.Error(string.Format("GameCenter TokenCheck failed, cause:{0}, usn:{1} {2}", resultString, platformId, auth.ToString()));
            return false;
        }
